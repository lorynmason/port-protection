import React from 'react';

export const Other = () => {
  return (
    <section id="other">
      <h1>Grocery Store</h1>
      <p>The grocery store is open 9:00am-5:00pm Tuesday-Friday</p>
      <p>SUMMER HOURS: 9:00am-5:00pm Monday-Friday</p>
      <p>If we are unable to open we will try to give an update on our <a href="">Facebook page</a> Port Protection Travel.</p>
      <h1>Fuel</h1>
      <p>HOURS: 9:00am-5:00pm Tuesday-Friday, 9:00am-1:00pm Saturday </p>
      <p>SUMMER HOURS: 9:00am-5:00pm Monday-Friday, 9:00am-1:00pm Saturday</p>
      <p>FUEL TYPES: Diesel Number One, Diesel Number Two, and Gasoline</p>
      <h3>Got questions? Contact us <a href="https://portprotectionlodge.typeform.com/to/EgWbs7">here</a></h3>
    </section>
  )
}